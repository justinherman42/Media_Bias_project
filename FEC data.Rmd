...
weekly_donations <- all_ceo %>% group_by(week = cut(contribution_receipt_date,"week"),committee_name) %>% summarise(value = sum(contribution_receipt_amount))

weekly_donations$csum <- ave(weekly_donations$value, weekly_donations$committee_name, FUN=cumsum)
# Basic barplot:
ggplot(weekly_donations, aes(x=committee_name, y=csum)) + 
