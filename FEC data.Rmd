...
# Basic barplot:
p <- ggplot(weekly_donations, aes(x=committee_name, y=csum,color=quarter)) + 
...
weekly_donations3 <- weekly_donations %>%
  mutate(show_time = case_when(week %in% c('2019-04-01',"2019-07-01") ~ 10,
                               TRUE           ~ 1)) %>%
...
ggplot(weekly_donations3, aes(x=committee_name, y=csum,color=quarter)) + 
  geom_bar(stat='identity')+
      theme(axis.text.x=element_text(angle=90,hjust=1))+
    labs(title="Cumulative CEO Campaign Donations {closest_state}") +
    transition_states(week)
...
unique(all_cand_df$committee_name)
...
old_df <- old_df %>% 
  filter(X_ind=="IND")
print(dim(old_df))
build_cand_df <- function(csv){
  new_df <- as.data.frame(read.csv(csv), stringsAsFactors = FALSE)
  new_df <- new_df %>% 
    filter(X_ind=="IND")
  print(dim(new_df))
  old_df <- as.data.frame(rbind(old_df,new_df),stringsAsFactors = FALSE)
}

#big_df <- apply(build_cand_df,list_csv)
bigger_df <- as.data.frame(big_df)
for (x in list_csv){
...
final_df <- old_df %>% 
  select(comm_id,Name1,Name2,address,city,state,zipcode,Date,donation_amt,donation_to_date_total,unkown2,employer,job_position)
...
colnames(new_tab_df) <- c("Different_CEO_Titles","Count")
kable(new_tab_df,caption = 'Top4126 uses of CEO with original unedited job title.')

a <- '20190831'

paste(substring(a,1,4),substring(a,5,6),substring(a,7,8),sep="/")
