...
##Full table- Not correct gifi
```{r,echo=F}
all_corporate_influence <- df_5 %>% 
    filter(df_5$clustered_jobs%in%c("CEO"))
...
```{r}
all_corporate_influence <- df_5 %>% 
    filter(df_5$clustered_jobs%in%c("C LEVEL_eXECS"))
allnames <- data.frame(rep(unique(all_ceo$comm_id),27))
build_dates <- rep(seq(as.Date("2018-12-31	"), as.Date("2019-07-01	"), by = "week"),13)
build_dates <- sort(as.Date(build_dates))
cart_df <- as.data.frame(cbind(allnames,build_dates))
...
p3 <- ggplot(weekly_donations, aes(x=comm_id, y=Money_Raised_From_CEO)) + 
  geom_bar(stat='identity')+
      theme(axis.text.x=element_text(angle=90,hjust=1))+
    labs(title="Cumulative C level executives {closest_state}") +
    transition_states(Date)
...
```{r}
animated_donation_by_job <- function(job_positions,graph_title)
{  
  all_corporate_influence <- df_5 %>% 
    filter(df_5$clustered_jobs%in%c(job_positions))
allnames <- data.frame(rep(unique(all_ceo$comm_id),27))
build_dates <- rep(seq(as.Date("2018-12-31	"), as.Date("2019-07-01	"), by = "week"),13)
build_dates <- sort(as.Date(build_dates))
cart_df <- as.data.frame(cbind(allnames,build_dates))
...
p3 <- ggplot(weekly_donations, aes(x=comm_id, y=Money_Raised_From_CEO)) + 
  geom_bar(stat='identity')+
      theme(axis.text.x=element_text(angle=90,hjust=1))+
    labs(title=paste(graph_title," {closest_state}")) +
    transition_states(Date)
